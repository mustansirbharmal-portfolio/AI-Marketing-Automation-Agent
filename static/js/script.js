document.addEventListener('DOMContentLoaded', () => {
    ...
    let campaign_id = localStorage.getItem('campaign_Id'); // Retrieve stored campaign ID
    ...
    if (msg.length === 0) {
        ...
        appendMessage(`Please Enter your Campaign Id`, 'bot-message', false);
    }
    else {
    ...
    function sendUserMessage(message) {
        // Store the campaign Id if it's the first user message
        if (msg.length === 1 && !campaign_id) {
           // Send the campaign ID to the backend to store it in the session
        fetch('/store_campaign_id', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ campaign_id: message })
        })
        .then(response => {
         
            // Append the success message for campaign ID
            appendMessage(` Campaign Id stored successfully`, 'bot-message', true);
            appendMessage(` How can I assist you today ?`, 'bot-message', true);
        })
        .catch(error => {
            ...
            fetch('/ask', {
                ...
                body: JSON.stringify({ query: message, campaign_id: localStorage.getItem('campaign_Id') })
            })
